...
// Pin Map (UNO)
constexpr uint8_t PIN_ARM           = 2; // toggle switch, INPUT_PULLUP (LOW=ON)
constexpr uint8_t PIN_RESET         = 3; // momentary switch, INPUT_PULLUP (LOW=pressed)
constexpr uint8_t PIN_LAUNCH        = 4; // momentary switch, INPUT_PULLUP (LOW=pressed)
constexpr uint8_t PIN_LED_READY     = 5; // led
constexpr uint8_t PIN_LED_ARMED     = 6; // led
constexpr uint8_t PIN_LAUNCH_LIGHT  = 7; // led
constexpr uint8_t PIN_RELAY         = 8; // igniter relay driver input
constexpr uint8_t PIN_BUZZER        = 9; // passive buzzer (tone())

// Relay polarity: I need to update this to match the hardware
// Discrete NPN driver -> ACTIVE = HIGH
// Many HiLetgo modules -> ACTIVE = LOW (flip these)
constexpr bool RELAY_ACTIVE = HIGH;
constexpr bool RELAY_INACTIVE = LOW;

// 16x2 LCD in 4-bit mode on analog pins
constexpr uint8_t LCD_RS = A0, LCD_E = A1, LCD_D4 = A2, LCD_D5 = A3, LCD_D6 = A4, LCD_D7 = A5;
LiquidCrystal lcd(LCD_RS, LCD_E, LCD_D4, LCD_D5, LCD_D6, LCD_D7);

// Timings (ms)
constexpr uint32_t HOLD_TO_LAUNCH_MS        = 5000; // must hold LAUNCH this long
constexpr uint32_t RELAY_ON_MS              = 5000; // relay energized duration
constexpr uint32_t COOLDOWN_MS              = 5000; // post-fire inhibit
constexpr uint32_t ABORT_INHIBIT_MS         = 1500; // after abort
constexpr uint32_t RESET_HOLD_MS            = 2500; // disarm + hold reset to clear FAULT
constexpr uint8_t STARTUP_CHECKS_COUNT      = 20;
constexpr uint32_t STARTUP_TOTAL_TIME_MS    = 5000; // 10s total
constexpr uint32_t STARTUP_CHECK_INTERVAL   = STARTUP_TOTAL_TIME_MS / STARTUP_CHECKS_COUNT;
...
/**
 * Plays a sequence of buzzer notes
 * 
 * @param seq Pointer to array of BuzzNote structures defining the sequence
 * @param len Number of notes in the sequence
 * @param loop Whether to loop the sequence continuously (default: false)
 */
...
/**
 * Stops the buzzer and deactivates the current sequence
 * 
 * @param pin The buzzer pin to stop
 */
...
/**
 * Updates the buzzer state and advances through the current sequence
 * 
 * @param now Current time in milliseconds
 * @param pin The buzzer pin to control
 */
...
/**
 * Checks if the ARM button is pressed (LOW = armed)
 * 
 * @return true if armed, false if disarmed
 */
inline bool armOn() { return dbArm.read() == LOW; }

/**
 * Checks if the RESET button is pressed
 * 
 * @return true if reset button is pressed, false otherwise
 */
...
/**
 * Checks if the LAUNCH button is pressed
 * 
 * @return true if launch button is pressed, false otherwise
 */
 ...
 * 
 * @param on true to activate relay, false to deactivate
 */
...
/**
 * Sets all output pins to their specified states
 * 
 * @param readyLed true to turn on ready LED, false to turn off
 * @param armedLed true to turn on armed LED, false to turn off
 * @param launchLamp true to turn on launch light, false to turn off
 * @param relayOn true to activate relay, false to deactivate
 */
...
/**
 * Updates the LCD display with two lines of text
 * 
 * @param l1 First line text to display
 * @param l2 Second line text to display
 */
...
///////////////////////////////
/**
 * Checks if a global fault condition exists
 * 
 * @return true if fault is active, false if system is normal
 */
...
///
/**
 * Transitions the system to a new state and performs state-specific initialization
 * 
 * @param s The new state to enter
 */
void enter(State s)
{
    state = s;
...
/**
 * Arduino setup function - initializes pins, LCD, and enters initial state
 */
void setup()
{
...
/**
 * Arduino main loop function - handles state machine, button inputs, and buzzer updates
 */
void loop()
{
    dbArm.update();
